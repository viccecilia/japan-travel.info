...
$pdo->prepare('INSERT INTO member_user (email, password_hash, referral_code, created_at) VALUES (?, ?, ?, ?)')
    ->execute(['test@example.com', password_hash('Password1234', PASSWORD_DEFAULT), 'JTTEST01', gmdate('c')]);
$userId = (int)$pdo->lastInsertId();
$token = jt_create_token($userId, 'email_verification', 300);
...
$allowed = '{"route":"https://example.rezio.shop/products/demo?foo=1"}';
putenv('REZIO_ROUTE_URLS_JSON=' . $allowed);
$urls = jt_allowed_rezio_urls();
ok(isset($urls['route']), 'rezio.shop host is allowed');
$withQuery = jt_url_with_query($urls['route'], ['click_id' => 'clk_1', 'utm_source' => 'site']);
ok(str_contains($withQuery, 'foo=1') && str_contains($withQuery, 'click_id=clk_1'), 'rezio query preserved and appended');
